...
async function loadJuz(juzNumber) {
  ...
  try {
    ...
    if (error.message.includes("Failed to fetch")) {
      ...
      errorMessage =
        "Format data tidak valid. Mohon periksa struktur file JSON.";
    }
  ...
  juzData.verses.forEach((verse) => {
    ...
    verseElement.innerHTML = `
            <div class="verse-header">
                ...
                ${
                  metadata
                    ? `<span class="verse-metadata">${metadata}</span>`
                    : ""
                }
            </div>
            <div class="verse-content">
                ...
                ${
                  verse.notes
                    ? `<div class="verse-notes">${verse.notes}</div>`
                    : ""
                }
                ${
                  verse.asbab && verse.asbab !== "0"
                    ? `<div class="verse-asbab">Asbabun Nuzul: ${verse.asbab}</div>`
                    : ""
                }
            </div>
        `;
